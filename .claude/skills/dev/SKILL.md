---
name: dev
description: Pick the highest priority task from the sprint backlog and implement it with TDD, security scanning, and browser-based E2E testing. Use when ready to implement the next task.
...
If `$ARGUMENTS` specifies a task number, work on that task instead.
